...
def save_frames_single_camera(camera_name):
    ...
    if not os.path.exists('../config/frames'):
        os.mkdir('../config/frames')
...
def save_camera_intrinsics(camera_matrix, distortion_coefs, camera_name):
    ...
    for l in camera_matrix:  # noqa: E741
        for en in l:
            outf.write(str(en) + ' ')
        outf.write('\n')
...
def check_calibration(camera0_name, camera0_data, camera1_name, camera1_data, _zshift):
    ...
    # dist1 = np.array(camera1_data[1])  # noqa: E265
    ...
    # Note that this uses homogeneous coordinate formulation
...
def save_extrinsic_calibration_parameters(R0, T0, R1, T1, prefix=''):
    ...
    outf.write('R:\n')
    for l in R0:  # noqa: E741
        for en in l:
            outf.write(str(en) + ' ')
        outf.write('\n')

    outf.write('T:\n')
    for l in T0:  # noqa: E741
        for en in l:
            outf.write(str(en) + ' ')
        outf.write('\n')
    outf.close()
    ...
    outf.write('R:\n')
    for l in R1:  # noqa: E741
        for en in l:
            outf.write(str(en) + ' ')
        outf.write('\n')

    outf.write('T:\n')
    for l in T1:  # noqa: E741
        for en in l:
            outf.write(str(en) + ' ')
        outf.write('\n')
    outf.close()
...
def save_to_yaml(K_0, K_1, d_0, d_1, R_0, t_0, R_1, t_1, e_0, e_1, e_stereo, prefix=''):
    # save data as yaml file
    # cam_0 projection matrix
    P_0 = np.eye(3, 4)
    P_0[:3, :3] = R_0
    P_0[:3, 3] = t_0.ravel()

    # cam_1 projection matrix
    P_1 = np.eye(3, 4)
    P_1[:3, :3] = R_1
    P_1[:3, 3] = t_1.ravel()
    ...
    # Config files path
    cam_0_params_path = os.path.join(dirname, '../config', prefix + 'cam_0_info.yaml')
    cam_1_params_path = os.path.join(dirname, '../config', prefix + 'cam_1_info.yaml')

    msg_0 = string_yaml(prefix + "cam_0", d_0, K_0, P_0, [640, 480], e_0, e_stereo)
    msg_1 = string_yaml(prefix + "cam_1", d_1, K_1, P_1, [640, 480], e_1, e_stereo)

    with open(cam_0_params_path, 'w') as f0:
        f0.write(msg_0)

    with open(cam_1_params_path, 'w') as f1:
        f1.write(msg_1)
...
def string_yaml(name, d, k, p, size, rmse, rmse_stereo):
    dist_model = "plum_bob"
    ...
    yaml_msg = "\n".join([
        ...
        "  cols: 4",
        "  data: " + format_mat(p, 5),
        "single camera reprojection error (mean squared):",
        "  data: " + str(rmse),
        "stereo reprojection error (mean squared):",
        "  data: " + str(rmse_stereo),
        ""
    ])
    return yaml_msg
...
if __name__ == '__main__':
    ...
    # camera0 intrinsics
    images_prefix = os.path.join('../config/frames', 'camera0*')
    cmtx0, dist0, rmse_0 = calibrate_camera_for_intrinsic_parameters(images_prefix)
    # save_camera_intrinsics(cmtx0, dist0, 'camera0') # this will write cmtx and dist to disk
    # camera1 intrinsics
    images_prefix = os.path.join('../config/frames', 'camera1*')
    cmtx1, dist1, rmse_1 = calibrate_camera_for_intrinsic_parameters(images_prefix)
    ...
    frames_prefix_c0 = os.path.join('../config/frames_pair', 'camera0*')
    frames_prefix_c1 = os.path.join('../config/frames_pair', 'camera1*')
    R, T, rmse_stereo = stereo_calibrate(cmtx0, dist0, cmtx1, dist1, frames_prefix_c0, frames_prefix_c1)
    ...
    """Optional: Save the calibration data in 2 yaml files"""
    save_to_yaml(cmtx0, cmtx1, dist0, dist1, R0, T0, R1, T1, rmse_0, rmse_1, rmse_stereo)

    """Optional: check your calibration makes sense """
    # camera0_data = [cmtx0, dist0, R0, T0]
    # camera1_data = [cmtx1, dist1, R1, T1]
    # check_calibration('camera0', camera0_data, 'camera1', camera1_data, _zshift = 13.)

    """Optional. Define a different origin point and save the calibration data"""
    # get the world to camera0 rotation and translation
    R_W0, T_W0 = get_world_space_origin(cmtx0, dist0, os.path.join('../config/frames_pair', 'test_int_frame_0.png'))

    # get rotation and translation from world directly to camera1
    R_W1, T_W1 = get_cam1_to_world_transforms(cmtx0, dist0, R_W0, T_W0,
                                              cmtx1, dist1, R1, T1,
                                              os.path.join('../config/frames_pair', 'test_int_frame_0.png'),
                                              os.path.join('../config/frames_pair', 'test_int_frame_1.png'),)
    ...
    save_to_yaml(cmtx0, cmtx1, dist0, dist1, R_W0, T_W0, R_W1, T_W1, rmse_0, rmse_1, rmse_stereo, prefix='world_to_')
